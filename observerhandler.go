...
// endpointTypeOf names the kind of resource an endpoint describes, for logs and metrics.
// Details is optional on an Endpoint, so it is not always known.
func endpointTypeOf(e observer.Endpoint) string {
	if e.Details == nil {
		return "unknown"
	}
	return string(e.Details.Type())
}
...
func (oh *observerHandler) OnAdd(added []observer.Endpoint) {
	oh.Lock()
	defer oh.Unlock()
	oh.onAddLocked(added)
}

// onAddLocked handles OnAdd while oh's mutex is already held.
func (oh *observerHandler) onAddLocked(added []observer.Endpoint) {
	for _, e := range added {
		...
		if !matched {
			oh.params.Logger.Info("endpoint did not match any exporter template",
				zap.String("endpoint_id", string(e.ID)),
				zap.String("endpoint_target", e.Target),
				zap.String("endpoint_type", endpointType),
			)
...
func (oh *observerHandler) OnRemove(removed []observer.Endpoint) {
	oh.Lock()
	defer oh.Unlock()
	oh.onRemoveLocked(removed)
}

// onRemoveLocked handles OnRemove while oh's mutex is already held.
func (oh *observerHandler) onRemoveLocked(removed []observer.Endpoint) {
...
func (oh *observerHandler) OnChange(changed []observer.Endpoint) {
	...
	oh.onRemoveLocked(changed)
	oh.onAddLocked(changed)
}
...
func (oh *observerHandler) startExporter(template exporterTemplate, env observer.EndpointEnv, e observer.Endpoint) {
	...
	_, hadExplicitEndpoint := template.config[endpointConfigKey]
	...
	if hadExplicitEndpoint {
		if _, ok := resolvedConfig[endpointConfigKey]; !ok {
			oh.params.Logger.Error("endpoint template expanded to empty; not falling back to discovered target",
	...
	oh.router.AddExporter(e.ID, exporterInstance, routingEnv, template.id.Type().String())
